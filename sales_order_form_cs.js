...
define(["N/https", "N/search", "/SuiteScripts/lib/micromodal.min"], function(
  ...
  function initModal() {
    ...
    modalHTML += "</main>";
    modalHTML += '<footer class="modal__footer">';
    modalHTML +=
      '<button class="modal__btn modal__btn-primary" id="btnCalculate">Calculate</button>';
    modalHTML +=
            ...
            if (id !== "0") {
              search.lookupFields
                ...
                .then(function(originAddress) {
                  ...
                  const totalWeight = totalWeightEl.getAttribute(
                    "data-total-weight"
                  );
                  // Set countryCode
                  ...
                  if (shippingMethodId === "rlcarriers") {
                    ...
                    document.querySelector(
                      "#modal-shipping-method-content"
                    ).innerHTML = "<p>This shipping method not available.</p>";
                  }
                });
            } else {
              document.querySelector(
   ...
   * @param {*} weight
   */
  function getFreightRate(origin, destination, classItem, weight) {
        ...
        if (rateRequestResultSuccess == "true") {
          ...
          var rateResultServiceLevels =
            body.GetRateQuoteResponse.GetRateQuoteResult.Result.ServiceLevels
              .ServiceLevel;
          var rate = 0;
          console.log("rateResultServiceLevels", rateResultServiceLevels);
  ...
  function buildFormRLCarriers(origin, destination, weight) {
    ...
    result += "</div>";
    result += "<hr>";
    result += '<p><strong>Class:</strong> <select id="selectClass">';
    result +=
    ...
    result += "</select></p>";
    result += "<p><strong>Total Weight:</strong> " + weight + "</p>";
    result += "</div>";
