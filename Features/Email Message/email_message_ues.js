...
define(['N/ui/serverWidget'], function(serverWidget) {
	...
	function embedJS(context) {
		try {
			...
			// 					'customdeploy_search_email_rl'
			// 				);
			// 				url += '&keyword=' + encodeURIComponent(e);
			// 				var headers = { 'Content-Type': 'application/json' };
			// 				var response = nlapiRequestURL(url, null, headers, null);
			// 				var body = JSON.parse(response.body);
			// 				if (body.success) {
			// 					var data = body.data.map(function(item) {
			// 						return item.values;
			// 					});
			// 					t(data);
			// 				}
			...
			// 				o = '<p>' + e.type + ': <strong style="font-weight: bold;">' + e.name + '</strong></p>';
			// 			return (
			// 				(o += '<p>Email: <strong style="font-weight: bold;">' + e.info1 + '</strong></p>'),
			// 				(n.innerHTML = o),
			// 				n
			// 			);
			// 		},
			// 		onSelect: function(e) {
			// 			NS.jQuery('#recipientemail').val(e.info1);
			// 		}
			// 	});
			// });
